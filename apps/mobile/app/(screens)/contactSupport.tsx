...
const ContactSupportScreen = () => {
  ...
  const handleSendMessage = () => {
    if (!subject.trim() || !message.trim()) {
      Alert.alert("Missing Information", "Please provide a subject and your message.");
      return;
    }
    ...
    Linking.canOpenURL(mailtoUrl)
      .then(supported => {
        if (!supported) {
          Alert.alert("Error", "Cannot open email client. Please contact us at " + SUPPORT_EMAIL);
        } else {
      ...
      .catch(err => {
        ...
        Alert.alert("Error", "An unexpected error occurred. Please contact us at " + SUPPORT_EMAIL);
      });
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled" contentContainerStyle={{paddingBottom: 20}}>
        <Text style={styles.infoText}>
          Have a question or need help? Fill out the form below, or email us directly at <Text style={styles.emailLink} onPress={() => Linking.openURL(`mailto:${SUPPORT_EMAIL}`)}>{SUPPORT_EMAIL}</Text>.
        </Text>
