...
async function enrichEventListOptimized(
  ...
  for (const event of events) {
    ...
    if (event.coverPhotoStoragePaths && event.coverPhotoStoragePaths.length > 0) {
      try {
        enrichedEvent.coverPhotoUrls = await generateSignedUrlsForPaths(
          event.coverPhotoStoragePaths
        );
      } catch (err) {
 ...
 */
async function generateSignedUrlsForPaths(
  storagePaths: string[],
  ...
  if (requiredAccess === "edit" || requiredAccess === "admin") {
    if (eventData.hostId !== userId) {
      ...
      throw createError(
        ...
        `User ${userId} does not have permission to edit event ${eventId}.`
      );
    }
  }
...
async function handleEventCommentCascadeDeletion(
  ...
  try {
    ...
    if (currentDepth >= maxDepth) {
      logger.warn(
        `Max recursion depth ${maxDepth} reached for event comment cascade deletion. Comment: ${parentCommentId}, Event: ${eventId}`
      );
      return 0;
    }
    ...
    if (childCommentsQuery.empty) {
      logger.info(
        `No child comments found for parent event comment ${parentCommentId} in event ${eventId}`
      );
      return 0;
    }

    logger.info(
      `Found ${childCommentsQuery.size} child comments to cascade delete for parent ${parentCommentId} in event ${eventId}`
    );
    ...
    for (const childDoc of childCommentsQuery.docs) {
      ...
      logger.info(
        `Deleted event comment ${childCommentId} and ${childrenDeleted} of its descendants from event ${eventId}`
      );
    }

    logger.info(
      `Deleted ${totalDeleted} event comments in cascade from parent ${parentCommentId} in event ${eventId}`
    );
    return totalDeleted;
  } catch (error) {
    logger.error(
      `Error in event comment cascade deletion for comment ${parentCommentId} in event ${eventId}:`,
      error
    );
      ...
      if (eventData.daySpecificTimes) {
        for (const [date, times] of Object.entries(eventData.daySpecificTimes)) {
          if (!validateDateFormat(date)) {
            throw createError(
              ErrorCode.INVALID_ARGUMENT,
              `Invalid date format in daySpecificTimes: ${date}`
            );
          }
          if (!validateTimeFormat(times.startTime) || !validateTimeFormat(times.endTime)) {
            throw createError(
              ErrorCode.INVALID_ARGUMENT,
              `Invalid time format in daySpecificTimes for ${date}`
            );
          }
        }
      }
        ...
        try {
          ...
          const smsPromises = invitedUsersSnapshot.docs.map(async (userDoc) => {
            ...
            if (
              ...
              userData.smsPreferences?.eventInvites
            ) {
              try {
                const eventDate = new Date(eventData.eventDate);
    ...
    try {
      ...
      if (hostDoc.exists) {
        ...
        hostName =
          ...
          `${hostUserData?.firstName} ${hostUserData?.lastName}` ||
          hostName;
...
export const updateEvent = onCall(
  ...
  withAuth(
    async (request) => {
      ...
      // Regenerate searchable fields if title, description, or location are updated
      if (updates.title || updates.description || updates.location?.address) {
        const finalTitle = updates.title || currentEventData.title;
        const finalDescription =
          updates.description !== undefined ? updates.description : currentEventData.description;
        ...
        Object.assign(updatePayload, searchFields);
      }
      ...
      if (eventData.coverPhotoStoragePaths && eventData.coverPhotoStoragePaths.length > 0) {
        ...
        const gcsDeletePromises = eventData.coverPhotoStoragePaths.map((path) =>
          bucket
            ...
            .catch((err: any) => {
              logger.error(
                `Failed to delete cover photo ${path} from GCS for event ${eventId}:`,
                err.message
              );
        ...
        logger.info(
          `Attempted deletion for ${eventData.coverPhotoStoragePaths.length} GCS cover photos for event ${eventId}.`
        );
      }
  ...
  withAuth(
    async (request) => {
      ...
      if (!eventData.requireRsvp) {
        logger.info(
          `Event ${eventId} does not require RSVP. User ${uid} attempted to RSVP with status ${status}.`
        );
      ...
      if (eventData.rsvpDeadline) {
        try {
          ...
          logger.error(
            `Invalid RSVP deadline format for event ${eventId}: ${eventData.rsvpDeadline}`,
            dateError
          );
  ...
  withAuth(
    async (request) => {
      ...
      if (!commentDoc.exists) {
        throw createError(
          ErrorCode.NOT_FOUND,
          `Comment ${commentId} not found on event ${eventId}.`
        );
      }
      ...
      if (cascadeDeletedCount > 0) {
        logger.info(
          `Cascade deletion removed ${cascadeDeletedCount} child comments for parent comment ${commentId} in event ${eventId}`
        );
      }
      ...
      logger.info(
        `User ${uid} deleted comment ${commentId} and ${cascadeDeletedCount} child comments from event ${eventId}. Total deleted: ${totalDeleted}`
      );
...
async function fetchRawEventsPage(
  ...
  let query = baseQuery
    ...
    .orderBy(secondaryOrderByField, secondaryOrderByDirection);
...
export const searchEvents = onCall(
  ...
  withErrorHandling(async (request) => {
    ...
    if (searchQuery && searchQuery.trim()) {
      ...
      // We need to run multiple queries and combine results due to Firestore limitations
      // Each query searches a different field
      const queries: FirebaseFirestore.Query<FirebaseFirestore.DocumentData>[] = [];
      ...
      // Apply final limit
      const pageEvents = allEvents.slice(0, limit);
    ...
    // If no search query, use the regular pagination approach
    const startValues: [string, string] | undefined =
      lastEventDate && lastEventId ? [lastEventDate, lastEventId] : undefined;
    ...
    // Filter by location if provided (still needs to be done in memory)
