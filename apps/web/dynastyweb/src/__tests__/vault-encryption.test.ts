...
jest.mock('libsodium-wrappers-sumo', () => ({
  ...
  randombytes_buf: jest.fn((len: number) => new Uint8Array(len)),
  crypto_pwhash: jest.fn(() => new Uint8Array(32)),
  crypto_secretstream_xchacha20poly1305_keygen: jest.fn(() => new Uint8Array(32)),
  crypto_secretstream_xchacha20poly1305_init_push: jest.fn(() => ({
    state: {},
    header: new Uint8Array(24)
  })),
  crypto_secretstream_xchacha20poly1305_push: jest.fn((state, chunk, ad, tag) => new Uint8Array(chunk.length + 17)),
...
describe('Vault Encryption', () => {
  ...
  describe('WebVaultCryptoService', () => {
    ...
    test('encrypts and decrypts file correctly', async () => {
      ...
      // Encrypt
      const encrypted = await cryptoService.encryptFile(testFile, fileKey);
    ...
    test('handles large files with chunking', async () => {
      ...
      const encrypted = await cryptoService.encryptFile(largeFile, fileKey);
      
      expect(encrypted.success).toBe(true);
      expect(encrypted.metadata?.fileSize).toBe(largeData.length);
    ...
    test('fails decryption with wrong key', async () => {
      ...
      const encrypted = await cryptoService.encryptFile(testFile, correctKey);
    ...
    test('zeroes out sensitive data after use', async () => {
      ...
      await cryptoService.encryptFile(testFile, fileKey);
  ...
  describe('WebKeyRotationService', () => {
    ...
    test('handles rotation failure gracefully', async () => {
      ...
      rotationService.reEncryptItemsWithNewKey = jest.fn().mockRejectedValue(
        new Error('Re-encryption failed')
      );
    ...
    test('decrypts with any available key', async () => {
      ...
      keyManager.retrieveKeyByVersion = jest.fn()
  ...
  describe('Integration Tests', () => {
    test('complete encryption workflow', async () => {
      ...
      const encryptedFileKey = await cryptoService.encryptData(fileKey, masterKey);
      
      // Simulate retrieval
      const retrievedMasterKey = await keyManager.retrieveKey(`${userId}-vault-key`);
      const decryptedFileKey = await cryptoService.decryptData(
  ...
  describe('Security Tests', () => {
    test('prevents XSS in file names', async () => {
      ...
      const fileKey = cryptoService.generateFileKey();
      const encrypted = await cryptoService.encryptFile(maliciousFile, fileKey);
      
      expect(encrypted.metadata?.fileName).not.toContain('<script>');
      expect(encrypted.metadata?.fileName).not.toContain('</script>');
    });
    ...
    test('enforces file size limits', async () => {
      ...
      const fileKey = cryptoService.generateFileKey();
      const result = await cryptoService.encryptFile(oversizedFile, fileKey);
    ...
    test('validates encryption metadata integrity', async () => {
      ...
      if (encrypted.metadata) {
        encrypted.metadata.fileSize = 99999;
      }
    ...
    test('rate limits encryption operations', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        promises.push(cryptoService.encryptFile(file, fileKey));
      }
